...
export default function MenuCard({ name, categories, items, date, onClick, onEditClick, onDeleteClick }: MenuCardProps) {
  const [showMenu, setShowMenu] = useState(false);
  
  return (
    <div
      className="w-[232px] h-[177px] bg-white rounded-md shadow-sm p-4 flex flex-col justify-between cursor-pointer relative"
    >
      ...
      <div className="flex flex-row justify-between">
        ...
        <div className="cursor-pointer z-10" onClick={() => setShowMenu((prev) => !prev)}>
      ...
      <h1       onClick={() => {
        onClick && onClick();
      }} className="text-[18px] text-[#555] font-nunito">{name}</h1>
      ...
      <p className="font-nunito text-[12px] text-[#999]"       onClick={() => {
        onClick && onClick();
      }}>Created on {date}</p>
